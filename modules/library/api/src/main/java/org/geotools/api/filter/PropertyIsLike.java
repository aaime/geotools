...
/**
 * Filter operator that performs the equivalent of the SQL "{@code like}" operator on properties of
 * a feature. The {@code PropertyIsLike} element is intended to encode a character string comparison
 * operator with pattern matching. The pattern is defined by a combination of regular characters,
 * the {@link #getWildCard wildCard} character, the {@link #getSingleChar singleChar} character, and
 * the {@link #getEscape escape} character. The {@code wildCard} character matches zero or more
 * characters. The {@code singleChar} character matches exactly one character. The {@code escape}
 * character is used to escape the meaning of the {@code wildCard}, {@code singleChar} and {@code
 * escape} itself.
 *
...
public interface PropertyIsLike extends MultiValuedFilter {
     ...
     *
     * <p>The ability to match case is pending the Filter 2.0 specification.
     *
