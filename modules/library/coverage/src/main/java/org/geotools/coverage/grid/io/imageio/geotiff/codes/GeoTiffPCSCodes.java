...
public final class GeoTiffPCSCodes {
     ...
     * Private User Implementations Special Ranges: 1. For PCS utilizing GeogCS with code in range
     * 4201 through 4321: As far as is possible the PCS code will be of the format gggzz where ggg
     * is (geodetic datum code -4000) and zz is zone. 2. For PCS utilizing GeogCS with code out of
     * range 4201 through 4321 (i.e. geodetic datum code 6201 through 6319). PCS code 20xxx where
     * xxx is a sequential number. 3. Other: WGS72 / UTM northern hemisphere: 322zz where zz is UTM
     * zone number WGS72 / UTM southern hemisphere: 323zz where zz is UTM zone number WGS72BE / UTM
     * northern hemisphere: 324zz where zz is UTM zone number WGS72BE / UTM southern hemisphere:
     * 325zz where zz is UTM zone number WGS84 / UTM northern hemisphere: 326zz where zz is UTM zone
     * number WGS84 / UTM southern hemisphere: 327zz where zz is UTM zone number US State Plane
     * (NAD27): 267xx/320xx US State Plane (NAD83): 269xx/321xx Note: These are only a subset of the
     * possible values
     */
     ...
     * (Hotine) Oblique Mercator). Note that this is the standard method of measuring azimuth, but
     * is opposite the usual mathematical convention of positive indicating counter-clockwise.
     */
     ...
     * coordinate of the center. This is NOT the False Easting.
     */
     ...
     * Projection. Note that this is not necessarily the origin of the projection.
     */
     ...
     * of Projection. Note that this is not necessarily the origin of the projection.
     */
     ...
     * coordinate of the center. This is NOT the False Northing. NOTE this value is incorrectly
     * named at http://www.remotesensing.org/geotiff/spec/geotiff2.7.html#2.7
     */
     ...
     * coordinate transformation method used. Note: this does not include the definition of the
     * corresponding Geographic Coordinate System to which the projected CS is related; only the
     * transformation method is defined here. GeoKeys Required for "user-defined" Coordinate
     * Transformations: PCSCitationGeoKey (additional parameter geokeys depending on the Coord.
     * Trans. specified).
     */
     ...
     * code is provided to specify the projected coordinate system. GeoKey requirements for
     * "user-defined" PCS families: PCSCitationGeoKey ProjectionGeoKey
     */
     ...
     * coordinate transformation method and projection zone parameters. Note : when associated with
     * an appropriate Geographic Coordinate System, this forms a Projected Coordinate System.
     * GeoKeys Required for "user-defined" Projections: PCSCitationGeoKey ProjCoordTransGeoKey
     * ProjLinearUnitsGeoKey (additional parameters depending on ProjCoordTransGeoKey).
     */
     ...
     * easting coordinate of the false origin. This is NOT the False Easting, which is the easting
     * attached to the Natural origin.
     */
     ...
     * northing coordinate of the False origin. This is NOT the False Northing, which is the
     * northing attached to the Natural origin.
     */
    ...
    /**
     * ProjScaleAtCenterGeoKey Key ID = 3093 Type = DOUBLE Units: none Scale at Center. This is a
     ...
     * ProjScaleAtOriginGeoKey (Rev. 0.2) Scale at Natural Origin. This is a ratio, so no units are
     * required.
     */
     ...
     * ModelTypeGeocentric = 3 Geocentric (X,Y,Z) Coordinate System Notes: 1. ModelTypeGeographic
     * and ModelTypeProjected correspond to the FGDC metadata Geographic and Planar-Projected
