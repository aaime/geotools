...
public class GeneralBounds extends AbstractBounds implements Cloneable, Serializable {
     ...
     *
     * <p>Consider this reference as final; it is modified by {@link #clone} only.
     */
     ...
     *
     * <p>This code will recognize the following exceptions:
     *
     * <ul>
     *   <li>ordinates encoding isNil
     *   <li>ordinates encoding isEmpty
     * </ul>
     *
     * @throws IllegalArgumentException if an ordinate value in the minimum point is not less than
     *     or equal to the corresponding ordinate value in the maximum point.
     ...
     * @throws IllegalArgumentException if the target units are not compatible with the units of the
     ...
     * upper corner to {@linkplain Double#POSITIVE_INFINITY positive infinity}. The {@linkplain
     * #getCoordinateReferenceSystem coordinate reference system} (if any) stay unchanged.
     *
     * @since 2.2
     */
    ...
    /**
     * Sets all ordinate values to {@linkplain Double#NaN NaN}. The {@linkplain
     * #getCoordinateReferenceSystem coordinate reference system} (if any) stay unchanged.
     *
     * @since 2.2
     */
     ...
     *
     * <ul>
     *   <li>If <code>isNull() == true</code>, then <code>{@linkplain #isEmpty()} == true</code>
     *   <li>If <code>{@linkplain #isEmpty()} == false</code>, then <code>isNull() == false</code>
     *   <li>The converse of the above-cited rules are not always true.
     * </ul>
     *
     ...
     *
     * <p>If {@code relativeToLength} is set to {@code false}, the actual tolerance value for a
     * given dimension <var>i</var> is {@code eps}.
     *
     * <p>Relative tolerance value (as opposed to absolute tolerance value) help to workaround the
     * fact that tolerance value are CRS dependent. For example the tolerance value need to be
     * smaller for geographic CRS than for UTM projections, because the former typically has a range
     * of -180 to 180° while the later can have a range of thousands of meters.
     *
     * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For
     * performance reason, it will no be verified unless J2SE assertions are enabled.
     *
