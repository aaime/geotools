 ...
 *
 * <p>It is better to avoid this class if you can. This class exists primarily for compatibility with external data or
 * applications that assume (<var>longitude</var>, <var>latitude</var>) axis order no matter what the EPSG database
 * said, for example Shapefiles.
 *
 * <p>The axis order can be specified at construction time as an array of {@linkplain AxisDirection axis directions}. If
 * no such array is explicitly specified, then the default order is {@linkplain AxisDirection#EAST East},
 * {@linkplain AxisDirection#EAST_NORTH_EAST East-North-East}, {@linkplain AxisDirection#NORTH_EAST North-East},
 * {@linkplain AxisDirection#NORTH_NORTH_EAST North-North-East}, {@linkplain AxisDirection#NORTH North},
 * {@linkplain AxisDirection#UP Up}, {@linkplain AxisDirection#GEOCENTRIC_X Geocentric X},
 * {@linkplain AxisDirection#GEOCENTRIC_Y Geocentric Y}, {@linkplain AxisDirection#GEOCENTRIC_Z Geocentric Z},
 * {@linkplain AxisDirection#COLUMN_POSITIVE Column}, {@linkplain AxisDirection#ROW_POSITIVE Row},
 * {@linkplain AxisDirection#DISPLAY_RIGHT Display right}, {@linkplain AxisDirection#DISPLAY_UP Display up} and
 * {@linkplain AxisDirection#FUTURE Future}. This means that, for example, axis with East or West direction will be
 * placed before any axis with North or South direction. Axis directions not specified in the table (for example
 * {@link AxisDirection#OTHER OTHER}) will be ordered last. This is somewhat equivalent to the ordering of
 * {@link Double#NaN NaN} values in an array of {@code double}.
 *
 * <p><strong>Notes:</strong>
 *
 * <ul>
 *   <li>This class compares only the "{@linkplain AxisDirection#absolute absolute}" axis directions, so North and South
 *       are considered equivalent.
 *   <li>The default direction order may changes in future Geotools version in order to fit what appears to be the most
 *       common usage on the market.
 *   <li>The actual axis ordering is determined by the {@link #compare compare} method implementation. Subclasses may
 *       override this method if the want to provide a more sophesticated axis ordering.
 * </ul>
 *
 * <p>For some authority factories, an instance of this class can be obtained by passing a
 * {@link Hints#FORCE_LONGITUDE_FIRST_AXIS_ORDER FORCE_LONGITUDE_FIRST_AXIS_ORDER} hint to the <code>
 * {@linkplain ReferencingFactoryFinder#getCRSAuthorityFactory
 ...
 * </blockquote>
 *
 * This class is named <cite>ordered axis authority factory</cite> instead of something like <cite>longitude first axis
 * order</cite> because the axis order can be user-supplied. The (<var>longitude</var>, <var>latitude</var>) order just
 * appears to be the default one.
 *
 * <p>This class implements {@link CRSAuthorityFactory} only. For an implementation that supports also
 * {@link org.geotools.api.referencing.cs.CSAuthorityFactory}, see {@link OrderedAxisAuthorityFactory}.
 *
 * @since 2.2
 ...
 */
public class OrderedAxisCRSAuthorityFactory extends TransformedAuthorityFactory
        implements CRSAuthorityFactory, Comparator<CoordinateSystemAxis> {
    /**
     ...
     *
     * <p>REMINDER: If this array is modified, don't forget to update the class javadoc above.
     */
    ...
    /**
     * The rank to be given to each axis direction. The rank is stored at the indice corresponding to the direction
     * {@linkplain AxisDirection#ordinal ordinal} value.
     */
    protected final int[] directionRanks;
    /**
     * {@code true} if this authority factory should also force the axis to their standard direction. For example if
     * {@code true}, then axis with increasing values toward South will be converted to axis with increasing values
     * toward North. The default value is {@code false}.
     *
    ...
    /**
     * {@code true} if this authority factory should also force all angular units to decimal degrees and linear units to
     * meters. The default value is {@code false}.
     *
     * @see Hints#FORCE_STANDARD_AXIS_UNITS
     * @since 2.3
     */
    ...
    protected static boolean booleanValue(final Hints userHints, final Hints.Key key) {
        if (userHints != null) {
    ...
    /**
     * Computes the rank for every direction in the specified. The rank is stored in an array at the indice
     * corresponding to the direction {@linkplain AxisDirection#ordinal ordinal} value. This method is used by
     * constructors for computing the {@link #directionRanks} field.
     *
     * @throws IllegalArgumentException If at least two axis directions are colinear.
     */
    ...
    /**
     * Returns the rank for the specified axis. Any axis that were not specified at construction time will ordered after
     * all known axis.
     */
    ...
    /**
     * Compares two axis for order. This method is invoked automatically by the {@link #replace(CoordinateSystem)
     * replace} method for ordering the axis in a coordinate system. The default implementation orders the axis
     * according their {@linkplain CoordinateSystemAxis#getDirection direction}, using the direction table given at
     * {@linkplain #OrderedAxisAuthorityFactory(AbstractAuthorityFactory, Hints, AxisDirection[]) construction time}
     * (see also the class description). Subclasses may override this method if they want to define a more sophesticated
     * axis ordering.
     *
     * @param axis1 The first axis to compare.
     * @param axis2 The second axis to compare.
     * @return A negative integer if {@code axis1} should appears before {@code axis2}, or a positive number if
     *     {@code axis2} should appears before {@code axis1}, or 0 if the two axis are unordered one relative to the
     *     other.
     * @todo The argument type will be changed to {@link CoordinateSystemAxis} when we will be allowed to compile for
     *     J2SE 1.5.
     * @since 2.3
     */
    @Override
     ...
     * {@link #replace(CoordinateSystem)} method. The default implementation replaces the unit only if the
     * {@link Hints#FORCE_STANDARD_AXIS_UNITS FORCE_STANDARD_AXIS_UNITS} hint was specified as {@link Boolean#TRUE TRUE}
     * at construction time. In such case, the default substitution table is:
     *
     * <p>
     *
     * <ul>
     ...
     * {@link #replace(CoordinateSystem)} method. The default implementation replaces the direction only if the
     * {@link Hints#FORCE_STANDARD_AXIS_DIRECTIONS FORCE_STANDARD_AXIS_DIRECTIONS} hint was specified as
     * {@link Boolean#TRUE TRUE} at construction time. In such case, the default substitution table is as specified in
     * the {@link AxisDirection#absolute} method. Subclasses may override this method if they want to use a different
     * substitution table.
     *
     * @since 2.3
     */
