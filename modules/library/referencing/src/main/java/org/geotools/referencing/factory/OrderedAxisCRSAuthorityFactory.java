 ...
 *
 * <p>It is better to avoid this class if you can. This class exists primarily for compatibility
 * with external data or applications that assume (<var>longitude</var>, <var>latitude</var>) axis
 * order no matter what the EPSG database said, for example Shapefiles.
 *
 * <p>The axis order can be specified at construction time as an array of {@linkplain AxisDirection
 * axis directions}. If no such array is explicitly specified, then the default order is {@linkplain
 * AxisDirection#EAST East}, {@linkplain AxisDirection#EAST_NORTH_EAST East-North-East}, {@linkplain
 * AxisDirection#NORTH_EAST North-East}, {@linkplain AxisDirection#NORTH_NORTH_EAST
 * North-North-East}, {@linkplain AxisDirection#NORTH North}, {@linkplain AxisDirection#UP Up},
 * {@linkplain AxisDirection#GEOCENTRIC_X Geocentric X}, {@linkplain AxisDirection#GEOCENTRIC_Y
 * Geocentric Y}, {@linkplain AxisDirection#GEOCENTRIC_Z Geocentric Z}, {@linkplain
 * AxisDirection#COLUMN_POSITIVE Column}, {@linkplain AxisDirection#ROW_POSITIVE Row}, {@linkplain
 * AxisDirection#DISPLAY_RIGHT Display right}, {@linkplain AxisDirection#DISPLAY_UP Display up} and
 * {@linkplain AxisDirection#FUTURE Future}. This means that, for example, axis with East or West
 * direction will be placed before any axis with North or South direction. Axis directions not
 * specified in the table (for example {@link AxisDirection#OTHER OTHER}) will be ordered last. This
 * is somewhat equivalent to the ordering of {@link Double#NaN NaN} values in an array of {@code
 * double}.
 *
 ...
 *   <li>The default direction order may changes in future Geotools version in order to fit what
 *       appears to be the most common usage on the market.
 ...
 * </ul>
 *
 * <p>For some authority factories, an instance of this class can be obtained by passing a {@link
 * Hints#FORCE_LONGITUDE_FIRST_AXIS_ORDER FORCE_LONGITUDE_FIRST_AXIS_ORDER} hint to the <code>
 * {@linkplain ReferencingFactoryFinder#getCRSAuthorityFactory
 * FactoryFinder.getCRSAuthorityFactory}(...)</code> method. Whatever this hint is supported or not
...
public class OrderedAxisCRSAuthorityFactory extends TransformedAuthorityFactory
     ...
     * converted to axis with increasing values toward North. The default value is {@code false}.
     *
    ...
    /**
     * Compares two axis for order. This method is invoked automatically by the {@link
     * #replace(CoordinateSystem) replace} method for ordering the axis in a coordinate system. The
     * default implementation orders the axis according their {@linkplain
     * CoordinateSystemAxis#getDirection direction}, using the direction table given at {@linkplain
     * #OrderedAxisAuthorityFactory(AbstractAuthorityFactory, Hints, AxisDirection[]) construction
     * time} (see also the class description). Subclasses may override this method if they want to
     * define a more sophesticated axis ordering.
     *
     ...
     *     positive number if {@code axis2} should appears before {@code axis1}, or 0 if the two
     *     axis are unordered one relative to the other.
     * @todo The argument type will be changed to {@link CoordinateSystemAxis} when we will be
     ...
     * {@link #replace(CoordinateSystem)} method. The default implementation replaces the unit only
     * if the {@link Hints#FORCE_STANDARD_AXIS_UNITS FORCE_STANDARD_AXIS_UNITS} hint was specified
     * as {@link Boolean#TRUE TRUE} at construction time. In such case, the default substitution
     * table is:
     *
     * <p>
     *
