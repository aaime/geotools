 ...
 * MathTransform math transform} from set of {@linkplain GeneralParameterValue parameter values}.
 * Implementations of this class should be listed in the following file:
 *
 * <blockquote>
 *
 * <p>{@code META-INF/services/org.geotools.referencing.operation.MathTransformProvider}
 *
 * </blockquote>
 *
 * <p>The {@linkplain DefaultMathTransformFactory math transform factory} will parse this file in
 * order to gets all available providers on a system. If this file is bundle in many JAR files, the
...
public abstract class MathTransformProvider extends DefaultOperationMethod {
     ...
     * unchanged to the {@linkplain
     * DefaultOperationMethod#DefaultOperationMethod(Map,int,int,ParameterDescriptorGroup)
    ...
    /**
     * Creates a math transform from the specified group of parameter values. Subclasses can
     * implements this method as in the example below:
     *
     * <blockquote>
     *
     * <pre>
     ...
     * @throws FactoryException if the math transform can't be created for some other reason (for
     *     example a required file was not found).
    ...
    /**
     * Constructs an optional parameter descriptor from a set of alias. The parameter is identified
     * as with {@link #createDescriptor}.
     *
     * @param identifiers The parameter identifiers. Most contains at least one entry.
     * @param minimum The minimum parameter value, or {@link Double#NEGATIVE_INFINITY} if none.
     * @param maximum The maximum parameter value, or {@link Double#POSITIVE_INFINITY} if none.
    ...
    /**
     * Constructs a parameter group from a set of alias. The parameter group is identified by codes
     * provided by one or more authorities. Common authorities are {@link Citations#OGC OGC} and
     * {@link Citations#EPSG EPSG} for example.
     *
     * <p>Special rules:
     *
     * <ul>
     *   <li>The first entry in the {@code identifiers} array is the {@linkplain
     *       ParameterDescriptorGroup#getName primary name}.
     *   <li>If a an entry do not implements the {@link GenericName} interface, it is an {@linkplain
     *       ParameterDescriptorGroup#getIdentifiers identifiers}.
     *   <li>All others are {@linkplain ParameterDescriptorGroup#getAlias aliases}.
     * </ul>
     *
     ...
     * @param param The parameter to look for.
     * @param group The parameter value group to search into.
     * @return The requested parameter value, or {@code 0} if {@code param} is {@linkplain
     *     #createOptionalDescriptor optional} and the user didn't provided any value.
     * @throws ParameterNotFoundException if the parameter is not found.
     * @todo Move to the {@link org.geotools.parameter.Parameters} class.
     */
    ...
    /**
     * The result of a call to {@link MathTransformProvider#createMathTransform
     * createMathTransform}. This class encapsulates a reference to the {@linkplain #method
     * originating provider} as well as the {@linkplain #transform created math transform}. This
     * information is needed when a provider delegates the work to an other provider according the
     * parameter values. For example a generic instance of {@link
     ...
     * matching the supplied parameters, because those dimensions determine the set of legal <code>
     * "elt_<var>j</var>_<var>i</var>"</code> parameters.
     *
     * <p>Most {@linkplain MathTransformProvider math transform provider} do not delegate their work
     * to an other one, and consequently do not need this class.
     *
     * <p>Future Geotools version may extends this class for handling more information than just the
     ...
     * provider to infer all the conditions prevaling when {@link
     * MathTransformProvider#createMathTransform createMathTransform} was executed. Furthermore, it
     * avoid to pollute {@code MathTransformProvider} with methods unused for the vast majority of
     * providers.
     *
     * @version $Id$
     * @author Martin Desruisseaux (IRD)
