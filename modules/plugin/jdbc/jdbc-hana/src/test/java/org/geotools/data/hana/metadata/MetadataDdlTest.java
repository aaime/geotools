...
public class MetadataDdlTest {
    ...
    public void testUomDdlGeneration() {
        ...
        Assert.assertEquals(
                "CREATE SPATIAL UNIT OF MEASURE \"degree\" TYPE ANGULAR CONVERT USING 0"
                        + ".017453292519943278",
    ...
    public void testSrsDdlGeneration() {
        ...
        Assert.assertEquals(
                "CREATE SPATIAL REFERENCE SYSTEM \"WGS 84\" IDENTIFIED BY 4326 ORGANIZATION "
                        + "\"EPSG\" IDENTIFIED BY 4326 DEFINITION 'GEOGCS[\"WGS 84\","
                        + "DATUM[\"WGS_1984\",SPHEROID[\"WGS 84\",6378137,298.257223563,"
                        + "AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],"
                        + "PRIMEM[\"Greenwich\",0,AUTHORITY[\"EPSG\",\"8901\"]],UNIT[\"degree\",0"
                        + ".0174532925199433,AUTHORITY[\"EPSG\",\"9122\"]],AUTHORITY[\"EPSG\","
                        + "\"4326\"]]' TRANSFORM DEFINITION '+proj=longlat +datum=WGS84 +no_defs' "
                        + "LINEAR UNIT OF MEASURE \"meter\" ANGULAR UNIT OF MEASURE \"degree\" TYPE"
                        + " ROUND EARTH ELLIPSOID SEMI MAJOR AXIS 6378137.0 INVERSE FLATTENING 298"
                        + ".257223563 COORDINATE LONGITUDE BETWEEN -180.0 AND 180.0 COORDINATE "
                        + "LATITUDE BETWEEN -90.0 AND 90.0 TOLERANCE 0  SNAP TO GRID 0  POLYGON "
                        + "FORMAT 'EvenOdd'  STORAGE FORMAT 'Mixed'",
                MetadataDdl.getSrsDdl(srs4326));
        Assert.assertEquals(
                "CREATE SPATIAL REFERENCE SYSTEM \"WGS 84 / Pseudo-Mercator\" IDENTIFIED BY 3857 "
                        + "ORGANIZATION \"EPSG\" IDENTIFIED BY 3857 DEFINITION 'PROJCS[\"WGS 84 / "
                        ...
                        + "+wktext +no_defs\"],AUTHORITY[\"EPSG\",\"3857\"]]' TRANSFORM DEFINITION "
                        + "'+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0"
                        + " +k=1.0 +units=m +nadgrids=@null +wktext +no_defs' LINEAR UNIT OF "
                        + "MEASURE \"meter\" TYPE PLANAR ELLIPSOID SEMI MAJOR AXIS 6378137.0 "
                        + "INVERSE FLATTENING 298.257223563 COORDINATE X BETWEEN -2"
                        + ".0037508342789248E7 AND 2.0037508342789248E7 COORDINATE Y BETWEEN -2"
                        + ".0048966104014635E7 AND 2.0048966104014624E7 TOLERANCE 0  SNAP TO GRID 0"
                        + "  POLYGON FORMAT 'EvenOdd'  STORAGE FORMAT 'Internal'",
                MetadataDdl.getSrsDdl(srs3857));
        Assert.assertEquals(
                "CREATE SPATIAL REFERENCE SYSTEM \"PRS92\" IDENTIFIED BY 4683 ORGANIZATION "
                        + "\"EPSG\" IDENTIFIED BY 4683 DEFINITION 'GEOGCS[\"PRS92\","
                        + "DATUM[\"Philippine_Reference_System_1992\",SPHEROID[\"Clarke 1866\","
